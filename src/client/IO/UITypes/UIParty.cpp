...
namespace jrc
{
    ...
    UIParty::UIParty()
        : UIDragElement<PosPARTY>(Point<int16_t>(300, 22)),
          background(300, 230, Geometry::BLACK, 0.80f),
          header(300, 22, Geometry::WHITE, 0.12f),
          divider(260, 1, Geometry::WHITE, 0.25f),
          title(Text::A12B, Text::LEFT, Text::WHITE, "Party"),
          party_info(Text::A11M, Text::LEFT, Text::WHITE, ""),
          invite_info(Text::A11M, Text::LEFT, Text::YELLOW, ""),
          invite_label(Text::A11M, Text::LEFT, Text::WHITE, "Invite:"),
          help_info(Text::A11M, Text::LEFT, Text::LIGHTGREY, "Tip: /party list, /party leader <name|id>"),
          party_id(-1),
          party_leader_id(-1),
          pending_party_invite_id(-1)
    {
        ...
        invite_field = Textfield(
            ...
            Text::LEFT,
            Text::WHITE,
            Rectangle<int16_t>(20, 226, 158, 176),
            13
        );
        invite_field.set_state(Textfield::NORMAL);
        invite_field.set_enter_callback([&](std::string) { send_invite(); });
        ...
        {
            set_party_state(
                statusbar->get_party_id(),
                statusbar->get_party_leader_id(),
                statusbar->get_party_members()
            );
    ...
    void UIParty::sync_text()
    {
        ...
        for (size_t i = 0; i < PARTY_SIZE; i++)
        {
            ...
            member_lines[i].change_text(
                member.name + " Lv." + std::to_string(member.level) + leader_tag + hp_tag
            );
        }
    }
